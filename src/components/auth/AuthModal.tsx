...
// ── OAuth Row ──────────────────────────────────────────────────────────────
function OAuthRow({ onOAuth, loading, disabled }: {
    onOAuth: () => void;
    loading: boolean;
    disabled: boolean;
}) {
    return (
        <div className="mb-4">
            <button
                ...
                className="w-full flex items-center justify-center gap-2.5 py-2.5 bg-white border border-slate-200 hover:border-slate-300 hover:bg-slate-50 rounded-xl transition-all shadow-sm active:scale-95 disabled:opacity-60 font-semibold text-slate-700 text-sm"
            >
                {loading ? <Loader2 size={16} className="animate-spin text-slate-400" /> : <GoogleLogo />}
...
function SignInForm({ onSuccess }: { onSuccess: () => void }) {
    ...
    const handleGoogle = async () => {
        ...
        if (error) { setError(error); setOauthLoading(false); }
        // On success the browser navigates away to Google
    };

    return (
        <div>
            <OAuthRow onOAuth={handleGoogle} loading={oauthLoading} disabled={isLoading} />
            {error && (
